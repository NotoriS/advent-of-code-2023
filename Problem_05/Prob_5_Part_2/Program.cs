...
public class Problem
{
    public static void Main(string[] args)
    {
        ...
        List<long> seedNumbers = lines.Dequeue().Remove(0, 7).Split(' ').ToList().Select(s => long.Parse(s)).ToList();
        List<List<long>> seedRanges = new List<List<long>>();
        for (int i = 0; i < seedNumbers.Count - 1; i += 2)
        {
            seedRanges.Add(new List<long> { seedNumbers[i], seedNumbers[i + 1] });
        }
        ...
        foreach (List<long> seedRange in seedRanges)
        {
